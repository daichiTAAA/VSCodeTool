    ...
    async invoke(options) {
        ...
        try {
            ...
            switch (p.operation ?? 'query') {
                case 'listEndpoints':
                    result = await g.vectorListEndpoints();
                    break;
                case 'getEndpoint':
                    result = await g.vectorGetEndpoint(p.endpointName ?? p.indexName);
                    break;
                case 'listIndexes':
                    result = await g.vectorListIndexes();
                    break;
                case 'getIndex':
                    result = await g.vectorGetIndex(p.indexName);
                    break;
                case 'nextPage':
                    result = await g.vectorQueryNextPage({ indexName: p.indexName, next_page_token: p.nextPageToken, query: p.query });
                    break;
                case 'scan':
                    result = await g.vectorScan(p.indexName, p.topK, undefined);
                    break;
                case 'query':
                default:
...
//# sourceMappingURL=vectorTool.js.map