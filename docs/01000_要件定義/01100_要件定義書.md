...
- **主要機能**:
  ...
  - 検索結果の整形・返却
  - Databricks API で利用可能なクエリID・インデックス名・GenieスペースID・Serving Endpoint名の定義
  - 定義に基づき利用する Databricks API を選択し Gateway API 経由で呼び出す

### 3.2.2 Gateway API
- **役割**: セキュアな Databricks API プロキシの提供
- **技術**: Python, FastAPI, OAuth2
- **主要機能**:
  - 認証・認可処理
  - Databricks API リファレンスと同一パス・同一メソッドでのエンドポイント提供
  - リクエストのプロキシ処理と監査ログ
  - レート制御

### 3.2.3 Azure Databricks
- **役割**: ナレッジベースの提供
- **技術**: Azure Databricks, Unity Catalog, SQL Warehouse, Vector Search, Serving Endpoints, Genie
- **主要機能**:
  - ナレッジデータの格納・管理
  - 検索ビューの提供
...
Gateway API は Databricks API リファレンスのエンドポイントを同一パス・同一メソッドで公開する。
主に利用するエンドポイントの例は以下の通り:
...
- `POST /api/2.0/serving-endpoints/{endpoint_name}/invocations` – Real-Time Serving Endpoints
...
**注記**: 本要件定義書は、添付された「LanguageModelToolAPIの使用検討」および「ナレッジベース情報取得に使用できそうなDatabricksAPI」資料を基に作成されており、技術的詳細については該当資料を参照してください。